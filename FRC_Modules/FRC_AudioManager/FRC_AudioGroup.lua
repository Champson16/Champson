...
function public:findFreeChannel(force)
	-- DEBUG:
	-- print("AudioGroup findFreeChannel searching through:");
	-- table.dump(self.channels);
	local channel;
	for i=1,#self.channels do
		...
		print "isChannelActive";
	end
	-- DEBUG
	print("AudioGroup findFreeChannel results:", channel);
	if (not channel) then
		if (#self.channels > 0) then
			...
			warn("AudioGroup " .. self.name .. " is attempting to force available another channel.");
...
function public:addChannel(increaseMax, startChannel)
	...
	local channel;
	local startSearchAtChannel = startChannel or self.startChannel;
	pcall(function() channel = audio.findFreeChannel(startSearchAtChannel); end);
	if (channel) then
...
function public:play(handleName, options)
	...
	if (type(handleName) == "string") then
		...
		if not handle then warn("A non-existant AudioHandle name was provided to audioGroup:play()."); end;
		-- TODO:  if 'handleName' is a string referencing a non-existent handle, the code breaks here
	else
		-- an actual handle instead of a handlename was passed in so handle it ;-)
		handle = handleName;
	end
...
function public:pause(handleName)
	if (handleName and type(handleName) == "string") then
		...
		-- DEBUG:
		print("AudioGroup:pause ALL CHANNELS");
		for i=1,#self.channels do
			print("pausing channel: ", self.channels[i]);
...
function public:disposeHandles()
	-- dispose of all audio handles that belong to this group
	for i=#self.handles,1,-1 do
		self.handles[i]:dispose();
		table.remove(self.handles, i);
...
function AudioGroup.new(options)
	...
	-- WIP to avoid stepping on top of VO channels
	audioGroup.startChannel = options.startChannel or 1;
	...
	for i=1,audioGroup.maxChannels do
		audioGroup:addChannel(false, audioGroup.startChannel + i - 1);
	end

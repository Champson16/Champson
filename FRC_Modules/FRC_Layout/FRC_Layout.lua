...
local getScreenDimensions = function()
	...
	if (build < 2109) then
		...
		--if (display.contentHeight < display.actualContentHeight) then
...
m.scaleToFit = function(displayObject, xOffset, yOffset)
	...
	local scaledSizeW = display.contentWidth * scale;
	local diffScaledW = ((refWidth * scale) - scaledSizeW) * 0.5;
	local diffW = ((refWidth * scale) - display.contentWidth) * 0.5;
	x = -(diffW - diffScaledW);
	
	local scaledSizeH = display.contentHeight * scale;
	local diffScaledH = ((refHeight * scale) - scaledSizeH) * 0.5;
	local diffH = ((refHeight * scale) - display.contentHeight) * 0.5;
	y = -(diffH - diffScaledH);

...
FRC_SceneManager.newScene = function(sceneName)
	...
	if (sceneName) then
		...
		table.insert(FRC_SceneManager.scenes, scene);
	end
	return scene;
end
...
FRC_SceneManager.loadScene = function(sceneName, data)
	assert(sceneName);
	local scene;
	for i=#FRC_SceneManager.scenes,1,-1 do
		if (FRC_SceneManager.scenes[i].sceneName == sceneName) then
			scene = FRC_SceneManager.scenes[i];
			break;
		end
	end
	if (not scene) then
...
FRC_SceneManager.gotoScene = function(sceneName, options)
	...
	for k,v in pairs(scene) do
		if (k == 'view') then scene.view.isVisible = true; break; end
	end
...
FRC_SceneManager.removeScene = function(sceneName, data)
	...
	if (scene) then
		for i=#FRC_SceneManager.scenes,1,-1 do
			if (FRC_SceneManager.scenes[i] == scene) then
				FRC_SceneManager.scenes[i] = nil;
				break;
			end
		end
